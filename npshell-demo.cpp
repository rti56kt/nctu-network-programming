...
void buildInCmd(vector<string> cmd){
    ...
    }else if(cmd.at(0).compare("unsetenv") == 0){
        if(cmd.size() == 2){
            char env_name[cmd.at(1).size() + 1];
            ...
            cerr << "usage: unsetenv [variable name]" << "\n";
        }
    }
...
void writeHistory(string user_input){
    int fd;
    string home_dir;

    home_dir = homeDir();
    fd = open((home_dir + "/.npshell_history").c_str(), O_RDWR | O_CREAT | O_APPEND, S_IRWXU | S_IRWXG | S_IRWXO);

    write(fd, (user_input + "\n").c_str(), user_input.size()+1);
...
void npsh(){
    ...
    while(true){
        ...
        writeHistory(user_input);

        if(user_input.size() >= 1) cmds = parseCmd(user_input, pipe_table, pipe_in_end);
        else continue;  // Means the input is a blank line

        if(cmds.at(0).at(0).compare("exit") == 0 || cmds.at(0).at(0).compare("setenv") == 0 || cmds.at(0).at(0).compare("printenv") == 0 || cmds.at(0).at(0).compare("unsetenv") == 0) buildInCmd(cmds.at(0));
