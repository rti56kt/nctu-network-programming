...
void initServer(int port){
    ...
    if(bind(msock, (struct sockaddr *)&master, sizeof(struct sockaddr_in)) == -1){
    ...
    while(1){
        int ssock = 0, ssocklen = 0, pid = 0;
        struct sockaddr_in slave;

        ssocklen = sizeof(struct sockaddr_in);
        ssock = accept(msock, (struct sockaddr *)&slave, (socklen_t *)&ssocklen);
        if(ssock == -1){
